...
class ProductoTest {
	...
	@Test
	void test_AtributosDinamicosValidos_SonConsideradosValidos() {
		pr1.setAtributoDinamico("Color","Rojo");
		pr1.setAtributoDinamico("Graficos Integrados", false);
		pr1.setAtributoDinamico("Disco", 500);
	...
	void test_registroDeItem_producto() {
		List<RegistroDeItem> registros = pr1.getRegistroDeItem(1.0);

		assertEquals(1, registros.size());
		assertEquals(pr1, registros.getFirst().getItem());
		assertEquals(90000.0, registros.getFirst().getPrecio(), 0.001);
	}
	
	@Test
	void test_NoPuedeAgregarItem() {
		Producto prMock = mock(Producto.class);

		assertThrows(UnsupportedOperationException.class, () -> pr1.add(prMock));
	}
	@Test
	void test_NoPuedeEliminarItem() {
		Producto prMock = mock(Producto.class);
